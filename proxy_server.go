...
func (s ProxyServer) clientToServerLoop() {
	...
	for {
		dataPacket, err := s.CoordinationSocket.Receive(DataCoordPacket)
		if err != nil {
			return
		}
	...
	for chunk := range s.ServerReader.Chunks {
		...
		for {
			...
			if err := dropSite.Upload(chunk); err != nil {
				errPacket := Packet{UploadErrorCoordPacket,
			...
			ack, err := s.CoordinationSocket.Receive(AckCoordPacket)
			if err != nil {
				return
...
func (s ProxyServer) allocDropSite() (int, bool) {
	if s.CoordinationSocket.Send(Packet{AllocDropSiteCoordPacket, nil}) != nil {
		return 0, false
	}
	response, err := s.CoordinationSocket.Receive(UseDropSiteCoordPacket)
	if err != nil {
		return 0, false
	}
